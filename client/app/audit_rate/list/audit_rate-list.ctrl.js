  ...
  function AuditRateCtrl($scope, $lcmaGrid, $lcmAlert, $lcmaDialog, $lcmaGridFilter, $uibModal, $lcmaPager, $lcmaPage, AuditRate) {
    ...
    var grid = _this.gridOptions = $lcmaGrid({
      ...
      .addCommandColumn('remove', 'remove', {
        cellTemplate: '<a class="ui-grid-cell-contents" ng-click="grid.appScope.removeAuditRate(row.entity, $index)"><i class="fa fa-trash"></i></a>'
      })
     ...
     */
    $scope.removeAuditRate = _this.removeAuditRate = function (audit_rate, index) {
      $lcmaDialog.confirm({
        titleText: 'Please confirm',
        bodyText: 'Are you sure you want to permanently remove this Audit Rate Table?'
